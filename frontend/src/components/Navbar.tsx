...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 sm:h-20">
          ...
          <Link href="/" className="flex items-center gap-2 group">
            ...
            <div className="flex flex-col gap-1">
              <span className={`text-base font-extrabold tracking-tight transition-colors duration-200 ${
                isScrolled ? "text-slate-900" : isDarkHeroPage ? "text-white" : "text-slate-900"
              }`}>
          ...
          <div className="hidden md:block">
            <button
              onClick={openModal}
              className={`px-5 py-2.5 rounded-lg text-sm font-bold tracking-wide flex items-center gap-1.5 transition-all duration-300 transform active:scale-95 shadow-md cursor-pointer ${
                isScrolled
          ...
          <div className="md:hidden flex items-center">
            <button
              ...
              className={`p-2 rounded-lg transition-colors focus:outline-none ${
                isScrolled ? "text-slate-800 hover:bg-slate-100" : isDarkHeroPage ? "text-white hover:bg-white/10" : "text-slate-800 hover:bg-slate-100"
              }`}
            >
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 top-16 z-30 bg-[#0a0516] flex flex-col md:hidden animate-fade-in border-t border-white/5">
              ...
              {navLinks.map((link, idx) => {
                ...
                return (
                  <Link
                    ...
                    className={`text-xl font-bold tracking-wide py-2 border-b border-white/5 transition-all duration-200 ${
                      isActive ? "text-primary pl-2 border-l-2 border-primary" : "text-slate-300 hover:text-white"
                    }`}
